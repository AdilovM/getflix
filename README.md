...
> An online database of information about movies. The user can create an account and keep track of movies watched.
...
3. In client directory, run webpack at local development mode:
  ```npm run build ```
4. In server directory, launch the local server:
  ```npm start ```
